...
const DropDownItem = styled.div`
  ...
  ${(props) =>
    ...
    `}
`;
const paymentMethods = ["Ethereum", "method 2", "method 3"];
...
export default function P2P() {
  ...
  //   const [pricePerUnitConversionRate, setPricePerUnitConversionRate] =
  //     useState("");
  ...
  useEffect(() => {
    if (mode == "Seller") {
      ...
      const becomeBuyer = async () => {
        try {
          setLoading(true);
          const url = "http://localhost:4000/user/becomeBuyer";
          const response = await fetch(url, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  useEffect(() => {
    if (pricePerUnitUSD == "" || pricePerUnitMATIC == "") {
      ...
      setPricePerUnitError(
        "Price Per Unit should be a positive number > zero!"
      );
    } else {
      setPricePerUnitError("");
    }
  ...
  const onChangePricePerUnit = (e, currency) => {
    if (currency == "usd") {
      ...
      setPricePerUnitMATIC(
        (pricePerUnitConversionRate * e.target.value).toFixed(3)
      );
    } else if (currency == "matic") {
      setPricePerUnitMATIC(e.target.value);
      setPricePerUnitUSD(
        ((1 / pricePerUnitConversionRate) * e.target.value).toFixed(3)
      );
    }
  };
  ...
  return (
    <div>
      ...
      <SideBar />
      <main
        className={`${styles.main} flex flex-col items-center justify-start ml-[8.2%]`}
      >
        {/* main heading */}
        <div className="font-barlow font-bold text-[56px] leading-[67px] text-white w-[90%] mt-[60px]">
          {mode == "Buyer"
            ? "Buy renewable energy"
            : mode == "Seller" && "Sell your renewable energy"}
        </div>
        {/* switch btn */}
        <div
          className="flex items-center justify-end mb-[30px] mt-[24px] w-[90%]"
          onClick={() =>
            mode == "Buyer"
              ? setMode("Seller")
              : mode == "Seller" && setMode("Buyer")
          }
        >
          <div className="font-montserrat font-bold text-[40px] leading-[120%] underline text-white cursor-pointer hover:text-green-500">
            {mode == "Buyer" ? "Sell" : mode == "Seller" && "Buy"}
          </div>
        </div>
        ...
        {mode == "Buyer" ? (
          <>
            ...
            <div className=" z-40 flex items-center justify-between w-[90%] mt-[64px]">
              ...
              <div className="flex items-center justify-between gap-[26px]">
                ...
                <div className="flex flex-col items-start justify-center gap-[16px]">
                  {/* text */}
                  <div className="font-montserrat font-bold text-[24px] leading-[29px] text-white">
                    Amount
                  </div>
                  {/* field */}
                  <input
                ...
                <div className="flex flex-col items-start justify-center gap-[16px]">
                  ...
                  <div
                    className={`${styles.userReviewRatingDropDown} ${
                      toggleDropdown == true && styles.selected
                    }`}
                    onClick={() => setToggleDropdown(!toggleDropdown)}
                  >
                    <div
                      className={`${styles.userReviewRatingDropDownText} ${
            ...
            <div className="z-20 flex items-center justify-between w-[90%] mt-[45px] mb-[10px]">
              <div className="font-montserrat font-bold text-white text-center text-[24px] leading-[29px] pl-[45px] w-[8%]">
                Wallet
                <br />
                Address
              </div>
              <div className="font-montserrat font-bold text-white text-center text-[24px] leading-[29px]">
                Price
                <br />
                (per unit)
              </div>
              ...
              <div className="font-montserrat font-bold text-white text-center text-[24px] leading-[29px] w-[23%]">
                Action
              </div>
            </div>
            ...
            {listings &&
              listings.map((listing, index) => {
                ...
                return (
                  ...
                  <div
                    key={index}
                    className={`${styles.dataRowContainer} z-20 flex items-center justify-between w-[90%] mt-[20px] py-[48px]`}
                  >
                    {/* Seller  */}
                    <div className="flex justify-center items-center font-montserrat font-normal text-white text-center text-[24px] leading-[29px] pl-[45px] w-[10%]">
                      {listing[0]?.slice(0, 4)}....${listing[0]?.slice(-4)}
                    </div>
                    {/* Price */}
                    <div className="flex justify-center items-center font-montserrat font-normal text-white text-center text-[24px] leading-[29px] w-[8%]">
                      {(
                        parseInt(listing[2].hex, 16) *
                        pricePerUnitConversionRate
                      ).toFixed(3)}{" "}
                      MATIC
                    </div>
          ...
          mode == "Seller" && (
            <>
              {/* form heading */}
              <div className="z-20 font-barlow font-medium text-white text-[48px] leading-[58px] w-[90%] pl-[10%] mt-[30px]">
                Enter details:
              </div>
              {/* field row */}
              ...
              <div className="z-20 flex items-center justify-between w-[70%] ml-[6%] mt-[36px]">
                {/* label */}
                <div className="font-barlow font-normal text-white text-[36px] leading-[43px]">
                  Price per unit (USD)
                </div>
              ...
              <div
                className={`${
                  pricePerUnitError == "" && "hidden"
                }  w-[60%] mt-[10px] text-right text-[red]`}
              >
                {pricePerUnitError}
              </div>
              ...
              <div
                className={`${
                  amountError == "" && "hidden"
                } w-[58%] mt-[10px] text-right text-[red] ml-[5px]`}
              >
                {amountError}
              </div>
              ...
              <div className="z-20 flex items-center justify-between w-[70%] ml-[6%] mt-[36px]">
                ...
                {/* Input */}
                <div className="flex items-center justify-center w-[60%]  border-[1px] rounded-[5px] py-[10px] font-barlow font-normal text-white text-[32px] leading-[38px]">
                  {(pricePerUnitUSD * amount).toFixed(2)} USD&nbsp; &nbsp;
                  &nbsp; &nbsp;&nbsp;{(pricePerUnitMATIC * amount).toFixed(2)}{" "}
                  MATIC
                </div>
              </div>
              ...
              <div
                className={`${
                  addedListingMessage == "" && "hidden"
                } mt-[10px] text-right text-[green]`}
              >
                {addedListingMessage}
              </div>
            </>
